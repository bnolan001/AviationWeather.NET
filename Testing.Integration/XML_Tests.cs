...
namespace Testing.Integration
{
    public class XML_Tests
    {
        ...
        {
            var request = _aviationWeather.GetPreviousObservationsAsync(new List<string>() { "WALL" }, 24);
            request.Wait();
            var obs = request.Result;
            obs.Should().NotBeNull();
            obs.Count.Should().Be(1);
        ...
        {
            var request = _aviationWeather.GetStationsInBoxAsync(39.83, -104.65, 41.0, -100.9);
            request.Wait();
            var stations = request.Result;
            stations.Should().NotBeEmpty();
